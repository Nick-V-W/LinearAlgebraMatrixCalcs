...
public class LinearAlgebraFunctions {
	...
	public static void pivotMath(float[][] matrix, int row, int pivPos, int col) {
		...
		for (int i = 0; i < matrix.length; i++) {
			if (i > row) {
				...
				elimRow(matrix, i, row, 1);
			} else if (i < row) {
				float scale = matrix[i][col];
				elimRow(matrix, i, row, scale);
			}
			printMatrix(matrix);
			System.out.println("----------------");
		}
	 ...
	 * @param pivRow
	 */
	private static void elimRow(float[][] rref, int rowToElim, int pivRow, float scale) {
		if (rref[rowToElim][pivRow] != 0) {
			for (int i = 0; i < rref[pivRow].length; i++) {
				rref[rowToElim][i] -= (rref[pivRow][i] * scale);
			}		
		}
